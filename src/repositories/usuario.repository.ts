...
const insertarUsuario = async (usuario: Usuario) => {
    const [fields] = await conn.promise().query<ResultSetHeader>('INSERT INTO usuarios (id, username, password) VALUES (?,?,?)',
    [
    500,
    usuario.username,
    usuario.password
...
const updateUsuario = async (usuario: Usuario) =>{
    const[fields] = await conn.promise().query<ResultSetHeader>(
        'UPDATE usuarios SET username = ?, password = ? WHERE id = ?', [usuario.username, usuario.password, usuario.id]
    );
    return fields.affectedRows;
